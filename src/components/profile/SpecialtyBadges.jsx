...
export default function SpecialtyBadges({ stats }) {
  ...
  const countByField = {
    plant:  (stats?.byCategory?.plant  || 0),
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-4 h-px" style={{ background: "rgba(232,224,208,0.3)" }} />
        <p className="text-[9px] font-black tracking-[0.4em] uppercase" style={{ color: "rgba(232,224,208,0.4)" }}>
          Specializations
        </p>
      </div>
        ...
        {SPECIALTIES.map(sp => {
          ...
          return (
            <div
              ...
              style={{ border: "1px solid var(--v1v-violet-border)", background: "var(--v1v-violet-bg)" }}
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0">
                  <p className="text-[9px] font-black uppercase tracking-[0.3em]" style={{ color: "var(--v1v-fg)" }}>{sp.name}</p>
                  <p className="text-[8px] uppercase tracking-wider mt-0.5" style={{ color: "var(--v1v-violet-border)" }}>
                    {currentLevel ? currentLevel.name : "Non débloqué"}
                  </p>
                </div>
                <p className="text-2xl font-black flex-shrink-0 number-display" style={{ color: "var(--v1v-violet)" }}>{count}</p>
              </div>

              <div className="h-px w-full mb-1" style={{ background: "rgba(124,58,237,0.12)" }}>
                <div
                  className="h-px transition-all"
                  style={{ width: `${Math.min(100, progress)}%`, background: progress >= 100 ? "var(--v1v-violet)" : "rgba(124,58,237,0.5)" }}
                />
              </div>
              {nextLevel && (
                <p className="text-[8px] tracking-widest uppercase" style={{ color: "rgba(124,58,237,0.45)" }}>
                  {count}/{nextLevel.min} — {nextLevel.name}
                </p>
              )}

              {isExpanded && (
                <div className="mt-3 pt-3 space-y-2" style={{ borderTop: "1px solid rgba(232,224,208,0.08)" }}>
                  {sp.levels.map(lvl => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <p className="text-[8px] tracking-widest uppercase" style={{ color: "var(--v1v-violet-border)" }}>
                            {lvl.min} req.
                          </p>
                          {unlocked && (
                            <div className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--v1v-violet)" }} />
                          )}
                        </div>
                      </div>
                    );
